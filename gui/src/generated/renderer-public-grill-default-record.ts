...
// To parse this data:
//
//   import { Convert, GrillDefaultRecord } from "./file";
//
//   const grillDefaultRecord = Convert.toGrillDefaultRecord(json);
//
 ...
 */
export interface GrillDefaultRecord {
    default:         Default;
    proposal_id:     string;
    question_id:     string;
...
export class Convert {
    public static toGrillDefaultRecord(json: string): GrillDefaultRecord {
        return validateP2c(cast(JSON.parse(json), r("GrillDefaultRecord")), p2cSchema, "GrillDefaultRecord");
    }

    public static grillDefaultRecordToJson(value: GrillDefaultRecord): string {
        return JSON.stringify(uncast(validateP2c(value, p2cSchema, "GrillDefaultRecord"), r("GrillDefaultRecord")), null, 2);
    }
}
...
const p2cSchema = {"$schema":"https://json-schema.org/draft/2020-12/schema","$id":"https://ananke.local/contracts/renderer-public-grill-default-record.schema.json","title":"GrillDefaultRecord","description":"Public immutable default record returned by record_grill_default. The deterministic evaluator supplies the default value.","type":"object","additionalProperties":false,"required":["proposal_id","revision","revision_hash","question_id","record_sequence","default","written_at","written_by"],"properties":{"proposal_id":{"type":"string","pattern":"^[a-z][a-z0-9_]{2,63}$"},"revision":{"type":"integer","minimum":1},"revision_hash":{"type":"string","pattern":"^sha256:[0-9a-f]{64}$"},"question_id":{"type":"string","pattern":"^grill_question_(observable_outcome|scope_compatibility|acceptance_evidence|destructive_external_authorization|adapter_worktree_isolation|autonomy_budget)$"},"record_sequence":{"type":"integer","minimum":1,"maximum":40},"default":{"type":"string","enum":["needs_rewrite","deny"]},"written_at":{"type":"string","x-ananke-utc-timestamp":true},"written_by":{"const":"deterministic_grill"}}};
const p2cRules = [
