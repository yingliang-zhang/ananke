...
// To parse this data:
//
//   import { Convert, GrillOverrideRecord } from "./file";
//
//   const grillOverrideRecord = Convert.toGrillOverrideRecord(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

/**
 * Public immutable waiver record returned by record_grill_override. Eligibility and the
 * waiver value remain deterministic bridge/store decisions.
 */
export interface GrillOverrideRecord {
    override:        Override;
    proposal_id:     string;
    question_id:     string;
...
export type Override = "waived";

export type WrittenBy = "local_gui_operator";

// Converts JSON strings to/from your types
// and asserts the results of JSON.parse at runtime
export class Convert {
    public static toGrillOverrideRecord(json: string): GrillOverrideRecord {
        return validateP2c(cast(JSON.parse(json), r("GrillOverrideRecord")), p2cSchema, "GrillOverrideRecord");
    }

    public static grillOverrideRecordToJson(value: GrillOverrideRecord): string {
        return JSON.stringify(uncast(validateP2c(value, p2cSchema, "GrillOverrideRecord"), r("GrillOverrideRecord")), null, 2);
    }
}
...
const p2cSchema = {"$schema":"https://json-schema.org/draft/2020-12/schema","$id":"https://ananke.local/contracts/renderer-public-grill-override-record.schema.json","title":"GrillOverrideRecord","description":"Public immutable waiver record returned by record_grill_override. Eligibility and the waiver value remain deterministic bridge/store decisions.","type":"object","additionalProperties":false,"required":["proposal_id","revision","revision_hash","question_id","record_sequence","override","written_at","written_by"],"properties":{"proposal_id":{"type":"string","pattern":"^[a-z][a-z0-9_]{2,63}$"},"revision":{"type":"integer","minimum":1},"revision_hash":{"type":"string","pattern":"^sha256:[0-9a-f]{64}$"},"question_id":{"type":"string","pattern":"^grill_question_(observable_outcome|scope_compatibility|acceptance_evidence|destructive_external_authorization|adapter_worktree_isolation|autonomy_budget)$"},"record_sequence":{"type":"integer","minimum":1,"maximum":40},"override":{"const":"waived"},"written_at":{"type":"string","x-ananke-utc-timestamp":true},"written_by":{"const":"local_gui_operator"}}};
const p2cRules = [
