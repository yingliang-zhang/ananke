...
// To parse this data:
//
//   import { Convert, RecordGrillDefaultInput } from "./file";
//
//   const recordGrillDefaultInput = Convert.toRecordGrillDefaultInput(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

/**
 * Public arguments of the future Tauri record_grill_default command. The default value is
 * selected from the persisted deterministic question; callers cannot supply it.
 */
export interface RecordGrillDefaultInput {
    proposal_id:   string;
    question_id:   string;
    revision:      number;
...
export class Convert {
    public static toRecordGrillDefaultInput(json: string): RecordGrillDefaultInput {
        return validateP2c(cast(JSON.parse(json), r("RecordGrillDefaultInput")), p2cSchema, "RecordGrillDefaultInput");
    }

    public static recordGrillDefaultInputToJson(value: RecordGrillDefaultInput): string {
        return JSON.stringify(uncast(validateP2c(value, p2cSchema, "RecordGrillDefaultInput"), r("RecordGrillDefaultInput")), null, 2);
    }
}
...
const p2cSchema = {"$schema":"https://json-schema.org/draft/2020-12/schema","$id":"https://ananke.local/contracts/renderer-public-grill-record-default-input.schema.json","title":"RecordGrillDefaultInput","description":"Public arguments of the future Tauri record_grill_default command. The default value is selected from the persisted deterministic question; callers cannot supply it.","type":"object","additionalProperties":false,"required":["proposal_id","revision","revision_hash","question_id"],"properties":{"proposal_id":{"type":"string","pattern":"^[a-z][a-z0-9_]{2,63}$"},"revision":{"type":"integer","minimum":1},"revision_hash":{"type":"string","pattern":"^sha256:[0-9a-f]{64}$"},"question_id":{"type":"string","pattern":"^grill_question_(observable_outcome|scope_compatibility|acceptance_evidence|destructive_external_authorization|adapter_worktree_isolation|autonomy_budget)$"}}};
const p2cRules = [
