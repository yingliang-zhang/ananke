...
- RED: `go test ./internal/trustedsupervisor -run '^TestProductionServerLifecycleGate' -count=1 -timeout=30s` failed to compile because the synchronized lifecycle gate/hooks did not exist. GREEN: the same focused suite passed after admission tracking and `WaitGroup.Add` moved under the gate used to mark shutdown; repeated `-count=50` and `-race -count=20` both passed.
- RED: `go test ./internal/trustedsupervisor -run '^TestProductionServerSigningMaterial' -count=1 -timeout=60s` failed to compile because mutable private-buffer decode hooks did not exist. GREEN: direct quoted-hex decoding now avoids a private Go string, retained aliases prove raw/encoded/decoded/key zeroization on error and Close, and repeated `-count=50` plus `-race -count=20` both passed.
...
- Lifecycle/key repair: focused suites passed at `-count=50`; the same suites passed under `-race -count=20`. The production secrecy path covering error text, logs, binary, journal, WAL/SHM, and public trust-bundle exposure passed at `-count=3`.
...
- A first lifecycle-budget repair separated the pre-Notify setup budget from a `540s + 5s + 120s = 665s` Notify-based execution budget. Compile/invariant/tagged-vet gates passed, but the second real canary still failed closed after `667.09s` with `lifecycle_timeout`; no terminal typed evidence was accepted. The remaining mismatch is architectural: snapshot materialization and invocation preparation occur after Notify but before the durable `running` event and before the executor starts its own `545s` process hard deadline.
- The next repair therefore uses a bounded two-stage wait: prelaunch waits only for signed `running` or terminal state; the `540s + 5s + terminal-persistence margin` clock starts only after `running` is observed. A bounded redacted state/attempt/time/failure-class timeline will distinguish prelaunch from running-terminal stalls without exposing paths, PIDs, sessions, hashes, evidence, model output, or credentials.
- The two-stage implementation passed tagged focused/compile/vet gates and full `trustedsupervisor` (`58.437s`). The third real canary failed closed after `182.51s` with `prelaunch_timeout`. Its bounded timeline contained exactly one signed event: sequence 1, attempt 1, state `prepared`, with no failure class and no `running` event.
- A separate 45-second diagnostic run produced a Go goroutine dump with no audit-executor run goroutine; the executor had exited while the test still polled. The preserved signed SQLite journal contained only the prepared event. Source tracing proved the root cause: safe pre-start `runErr` cleanup attempted `prepared -> failed`, but the journal transition table permits prepared only to running/cancelled/waiting-for-human; the rejected terminal append was discarded by `_ = executor.appendEvent`, stranding the durable execution in prepared. This is a production P5 state-machine/persistence defect, not another canary deadline mismatch.
- Repair now targets the whole class: legal pre-start waiting terminal, no silently ignored required terminal append, observable persistence failures, and regression coverage for runtime-authority/gateway/transport/start/cancellation failures. No fourth provider run is allowed until those gates pass.
- Production repair completed: a pre-running `runErr` with last durable state `prepared` now persists signed `waiting_for_human`; running failures retain the existing failed path. All required terminal appends are checked. If a requested terminal append fails while the prior state permits a safe fallback, the executor attempts a scrubbed `terminal_event_persistence_failed` waiting terminal; if journal persistence itself fails, no in-memory terminal is fabricated, `executor.Close()` returns the retained error, and restart recovery handles the signed prepared history.
- Regression coverage includes runtime-authority verification, provider-gateway setup, transport binding, start gate, command start, zero provider/process effect before start, raw-detail redaction, forced SQLite terminal-append failure, close observability, and restart recovery. Independent gates: focused **PASS** (`4.057s`), `-count=10` **PASS** (`25.646s`), race `-count=3` **PASS** (`24.073s`), full `trustedsupervisor` **PASS** (`64.199s`), vet/gofmt/diff-check **PASS**.
- The fourth real canary was launched only after these gates passed. It must either reach durable running/terminal evidence or reveal a closed pre-start failure class; prepared-only stranding is no longer an accepted outcome.
- Fourth real canary failed closed in `9.10s` with a correct signed sequence: `prepared -> waiting_for_human(provider_gateway_setup_failed)`. This proves the prepared-terminal repair works in the real path and localizes the next blocker before sandbox/OMP/provider effect.
- Live network checks found system DNS resolves exact allowlisted `coding.sudoai.cc:443` to `198.18.0.34`, inside RFC 2544 `198.18.0.0/15`; direct no-proxy HTTPS still returned HTTP 200 with remote IP `198.18.0.34` and TLS verification result `0`, proving a transparent local TUN fake-IP mapping. Dual IPv4/IPv6 loopback listener binding passed. The broker correctly rejected that reserved range under its public-IP-only rule.
- Next repair is narrow: retain public-IP checks, but separately accept all-fake `198.18/15` DNS answers only for an exact allowlisted provider route, reject mixed public/fake answers, keep exact-address pinning, `Proxy:nil`, original hostname/Host/SNI, certificate verification, redirect rejection, and all private/loopback/link-local/CGNAT/documentation/reserved denials. No global reserved-range relaxation is allowed.
- Transparent fake-IP repair completed without policy-schema/hash changes. `publicAuditBrokerAddress` remains strict; a separate Darwin audit-bound predicate accepts `198.18.0.0/15` only for an exact existing provider+endpoint route. Resolution rejects mixed classes, duplicates, zones, malformed/unbounded answers, and every other reserved class. Dial remains exact-IP pinned with `Proxy:nil`, original Host/SNI, hostname certificate validation, redirect rejection, and bounded request/response behavior.
- RED reproduced exact route rejection. GREEN: focused fake-IP/address/pinning/TLS **PASS** (`2.129s` independent); agent focused `-count=10` **PASS** (`5.957s`), race `-count=3` **PASS** (`3.916s`), full `trustedsupervisor` **PASS** (`53.796s`), tagged canary compile/invariant **PASS** (`0.490s`); independent vet/gofmt/diff-check **PASS**.
- Fifth real canary launched after these gates. Expected next proof is durable `running`; any failure must remain a closed signed timeline.
- Fifth real canary crossed the gateway and reached durable `running`. It then failed closed in `12.20s` with signed sequence `prepared -> running -> waiting_for_human(group_exit_unconfirmed)` and `executor_close_error`; no sandbox/OMP/test process remained in the post-run process scan.
- Source tracing shows the verified group leader exited and was reaped while a short-lived same-PGID descendant survived beyond the 750ms confirmation grace. The current code therefore stopped before collecting the wrapper exit result. Safety decision: never signal after the leader is reaped because PGID reuse creates a TOCTOU; instead add a separate bounded 5s natural residual-group observation grace with no signals. Confirm only actual group absence; otherwise retain `group_exit_unconfirmed`. Existing TERM→KILL remains only for a live exact-identity leader.
- Residual-group repair completed. `auditTerminationBounds` now has a separate positive, max-30s `ResidualExitGrace` with a 5s production default and a Close control bound that includes it. After a valid waiter proves the original leader exited and identity inspection proves it absent, the code only polls `GroupExists`; it never signals this path. Missing/incomplete/invalid waiter, inspection failure, persistent group, and context expiry remain typed fail-closed outcomes. Live exact-identity cancellation retains TERM→KILL.
- Regression coverage includes normal/nonzero leader exit, natural residual disappearance, persistent residual timeout, inspection failure, incomplete/bad waiter, missing-waiter immediate proof only, context cancellation, join path, invalid bounds, concurrent Close, and a real Darwin leader+short-lived child group with zero signals. Independent focused **PASS** (`2.530s`), `-count=10` **PASS** (`18.331s`), race `-count=3` **PASS** (`11.360s`), full `trustedsupervisor` **PASS** (`57.383s`), vet/gofmt/diff-check **PASS**.
- Sixth real canary launched after these gates. It should now either expose the closed wrapper exit class after actual group disappearance or continue through the provider audit.
- Sixth real canary again reached durable `running` but failed closed after the new grace: `prepared -> running -> waiting_for_human(group_exit_unconfirmed)` in `23.07s`; running-to-terminal was `6.766s`, proving the 5s residual observation executed but did not establish absence.
- Source audit found the structural cause. The pinned wrapper runs `set -m` before background OMP, placing OMP in a different PGID from the verified sandbox leader, while its watchdog uses a subshell plus external `sleep`. The supervisor therefore cannot claim complete containment from the sandbox PGID alone, and killing the watchdog shell can leave its sleep child. Decision: no further grace extension and no signal to a reaped/reusable PGID.
- Repair direction: bump the atomic runtime authority to a closed v2 single-group/watchdog-FD contract. The trusted bootstrap will intercept only `set -m/+m`, keeping wrapper/OMP in the sandbox group; an inherited read-only FD 3 plus Bash builtin `read -t` supplies a child-free watchdog delay; exact watchdog cleanup becomes SIGKILL and wait. Bootstrap/framing/FD/policy hashes remain release/policy bound. This closes the known wrapper-induced split; arbitrary untrusted `setsid` remains a separately declared P6 gate requiring exclusive dedicated-UID lease plus UID-empty terminal proof.
- P5 v2 implementation completed after the first continuation exposed a missing production wire: `audit_executor.go` now creates a second trusted pipe, passes exactly its reader as child FD 3, closes the parent reader after start, retains the unwritten writer until verified group exit, and transfers it with an unconfirmed pending process. The closed v2 runtime authority binds single-group policy, FD number/policy, bootstrap hash, and framed wrapper bytes; the external wrapper remains unchanged.
- Independent gates after gofmt: focused bootstrap/authority/watchdog/process-group/residual/prestart `-count=10` **PASS** (`58.209s`), race `-count=3` **PASS** (`20.624s`), full trusted-supervisor **PASS** (`74.921s`), vet/gofmt/diff-check **PASS**, tagged canary contract compile **PASS** (`0.892s`). Seventh real-provider canary launched without the previously blocked out-of-band process monitor.
- The first seventh-run command failed before setup because it incorrectly mapped then removed `SUDO_CODING_KEY`; the harness intentionally retains that name for policy binding and leak scans. Relaunch kept only `SUDO_CODING_KEY` and reached `prepared -> running -> waiting_for_human(group_exit_unconfirmed)` in `23.18s`.
- Immediate post-failure process evidence identified the exact residual: PID `55360`, PPID `1`, PGID `55346`, command `/bin/sleep 545`. Root cause was the bootstrap's sandbox-incompatible `[ -r /dev/fd/3 ]` path probe: inherited FD 3 existed, but the path check failed under Seatbelt and fell back to external sleep. The watchdog shell was killed while its child became orphaned. No group signal was sent after leader reap; the inert sleep is left to expire naturally.
- Repair removed all `/dev/fd/3` path use and directly invokes Bash builtin `read -t ... -u 3` on the policy-bound inherited descriptor. Missing/closed FD now exits `1` promptly instead of creating a fallback child. RED reproduced old hash plus exit-semantic mismatch; GREEN focused `-count=10` **PASS** (`13.997s`), race `-count=3` **PASS** (`13.939s`), full trusted-supervisor **PASS** (`61.977s`), vet/gofmt/diff-check **PASS**. Eighth real-provider canary launched.
- Eighth canary crossed the containment boundary: `prepared -> running -> failed(wrapper_exit_nonzero)` in `11.82s`, with no `group_exit_unconfirmed`. Ninth canary added a canary-only closed exit-code projection inside the existing five-field safe timeline and reproduced `wrapper_exit_nonzero_code_1` in `13.17s`; no raw output, path, PID, session, hash, evidence, or credential was logged.
- Source inspection found the next contract mismatch: policy admitted preferred source credential `SUDO_CODING_KEY`, but `auditInvocationEnvironmentWithLookup` passed that name unchanged while the OMP runtime consumes `SUDO_API_KEY`. Trusted child environment now deterministically projects `custom:sudo + SUDO_CODING_KEY` to `SUDO_API_KEY`, never carries the source-only name, ignores ambient non-selected `SUDO_API_KEY`, and preserves legacy `SUDO_API_KEY` identity mapping. Command descriptor already binds provider plus selected source name.
- Credential-projection focused `-count=10` **PASS** (`15.918s`), race `-count=3` **PASS** (`10.992s`), full trusted-supervisor **PASS** (`65.116s`), vet/gofmt/diff-check **PASS**. Tenth real-provider canary launched with only parent `SUDO_CODING_KEY`; the trusted child performs the one-time projection.
- Tenth canary is invalid as provider evidence: P6 Repair C was concurrently writing the same repository after the canary baseline, so the canary correctly failed `repository_immutability`; its simultaneous `wrapper_exit_nonzero_code_1` is not interpreted independently from that contaminated run. New operational gate: a real-provider canary requires an exclusive quiet source window with no concurrent coding agent or local edit on the target repository. Retry only after Repair C stops and a fresh pre/post status check proves stability.
- Eleventh canary ran in an exclusive quiet source window and reproduced `wrapper_exit_nonzero_code_1`. A closed, canary-only pre-cleanup diagnostic hook was added: it reads at most 64 KiB from wrapper stdout/stderr/output, checks credential disclosure first, reduces bytes to one compiled class, zeroes the sample, and adds only that class to the existing `failure_class` field. Unit `-count=10` **PASS** (`0.761s`), race `-count=3` **PASS** (`1.577s`), full trusted-supervisor **PASS** (`64.344s`). Twelfth and thirteenth quiet canaries classified the failure as `wrapper_exit_nonzero_code_1_sandbox_denied` in `11.81s` and `12.52s`.
- A clean 30-second unified-log window contained 39 relevant denies and no `forbidden-exec-sugid`. The unique likely-fatal deny was Bash `file-write-create` for basename `sh-thd-<dynamic>` under the shared OS temp parent, outside all invocation-owned prompt/output/session/tmp/home/agent/wrapper-state roots. This is Bash 3.2 materializing the frozen wrapper's Python heredoc before `snapshot_session_paths`; Seatbelt correctly rejects it.
- Direct experiments rejected the initial nested-exec hypothesis: root-owned `/bin/test` and pinned OMP `--version` both execute successfully as nested Bash children under `(allow default)`. Therefore widening process-exec is irrelevant. Broad write access to the shared OS temp root is rejected as an architecture regression (race, residue, cross-run authority).
- Decision: P5 runtime authority v3 must launch the pinned OMP executable as the initial sandbox target. The trusted supervisor will natively own exact route-to-model/thinking mapping, internal/hard timeout, output capture, session directory, process-group cleanup, and credential projection. The external Hermes wrapper remains unmodified and frozen as a compatibility oracle; its Bash/heredoc bytes are not executed inside Seatbelt. Production activation remains inert until direct-launch contract tests, full gates, and a new real canary pass.
...
- P5 runtime authority v3 now launches the pinned OMP executable directly as the initial Seatbelt target while retaining the external Hermes wrapper only as a frozen compatibility oracle. Three delegated implementation/resume attempts timed out; the orchestrator repaired the remaining contract regressions from 25 failing tests to 14 and then 0. The full trusted-supervisor package passed twice (`69.247s`, `62.405s`), and tagged canary compilation passed (`0.721s`). These changes remain uncommitted.
- The first direct-OMP quiet canaries reached durable `running` but failed closed as `direct_omp_exit_nonzero_code_1_native_addon_load`. Unified sandbox logs and a no-provider factorial probe isolated the root cause: real OMP requires `file-read-data` directory listing on the exact pinned executable/native ancestor chain; exact native-file read/map and native write denial were already sufficient. Probe arms without ancestor listing (`A`, `S`) reproduced `native_addon_load`, while ancestor-listing arms (`D`, `B`) loaded the addon and advanced to the expected dummy-provider timeout. Production now emits only exact literal ancestor-directory rules from the already verified atomic runtime authority; no broad runtime subpath or sysctl allowance was added.
- Post-repair real canaries no longer report native-addon failure. The latest quiet runs persisted `prepared -> running -> failed(direct_omp_or_capture_verification_failed_working_only)` in approximately 12–14 seconds. Closed transport telemetry reported `lookups=1`, `dial_attempts=0`, `dial_successes=0`; therefore no request reached the trusted provider gateway and no model evidence was accepted. Current sandbox evidence shows exact `/usr/bin/git` execution and snapshot-parent discovery attempts are denied before provider transport. The next repair must remain narrow: admit only the already policy-pinned Git executable, prevent parent/home Git discovery with fixed environment ceilings, and retain denial of shells, arbitrary executables, and unrelated network endpoints.

#### Decision

- **CANARY REMAINS OPEN:** provider authentication, durable running transition, direct pinned-OMP launch, process-group cleanup, credential projection, fake-IP broker admission, and native-addon loading are individually proven. End-to-end read-only audit is not yet proven: the current direct OMP runtime stops before its first trusted-gateway dial and returns no canonical approved/rejected report. Do not infer provider or audit success until the exact Git/startup boundary is repaired, focused/full gates pass, and a quiet real canary returns typed model evidence.
...
#### P6 Contract Slice 3 — candidate

- Added pure supervisor-intent contract for exactly three phases: `materialization_claim`, `adapter_claim`, and `test_claim`, each with a unique trusted `(attempt_hash, phase)` slot and `FULL+fullfsync` durability identity. No store, fsync, transport, launch, Git/worktree, sandbox, UID, signature, or production command was added.
- Claims bind authorization/approval/policy, complete P4/fence, request/dispatch/channel/peer, boot epoch, journal head/slot, repository/base, executable/sandbox/namespace/root, timestamps, predecessor claim/event, and canonical record hash. Empty slots yield only awaiting-commit/no-effect; exact committed replay yields opaque predecessor evidence/no-effect.
- Added a ten-cut-point recovery matrix. All successful recovery results have `effect_allowed=false`; prior-epoch nonterminal states wait for human, current-epoch observations are status-only, and persisted responses replay only the response.
- Added a separate ordered 64-vector executable registry and machine-parsed normative document with canonical attempt-1/attempt-2 claim-chain hashes.
- Independent gates: focused **PASS** (`2.338s`); package single **PASS** (`2.037s`); `-count=10` **PASS** (`17.315s`); race `-count=3` **PASS** (`51.882s`); vet and diff-check **PASS**.
- Frozen candidate: 29 files, manifest `sha256:8d99981593745eaf614e3609fbe993433176e7d09746ebcc38474d3c675c9816`. Fresh-context hard review is in progress; runtime journal implementation remains blocked pending verdict.

#### P6 Contract Slice 3 — first hard review

- Report: `artifacts/omp/p6a/slice3-first-review.md`, SHA-256 `97e39aafec40e48bc2c886f930fd29216b058c1c751a8d8315cde213fd54bd1d`; frozen 29-file manifest pre/post mismatches `0`.
- Verdict: **SLICE CHANGES REQUESTED**. Three HIGH findings share one root cause: public caller declarations were treated as trusted journal observations even though every returned effect bit remained false.
- HIGH 1: exported slot status/tuple/ID/committed bytes could mint a committed predecessor capability, accept a second slot for one tuple, alias one slot ID across phases, and accept an invented predecessor terminal-event hash. Required: opaque journal-minted slot/commit and terminal-event capabilities with private integrity/uniqueness state.
- HIGH 2: exact committed replay at exclusive `not_after` still minted a new predecessor capability. Historical bytes may classify as replay, but stale authorization/dispatch/release/claim state must not mint capability.
- HIGH 3: recovery durability was agreement between public hashes and caller booleans (`Complete`/`Ambiguous`), not verified canonical journal records. Required: opaque verified observations or closed self-hashed records bound to attempt/phase/claim/slot/boot/head/predecessor/durability/order.
- MEDIUM cleanup: remove exported impossible launch-action constants, replace decorative duplicate probes with real alias sequences, and remove map-order nondeterminism.
- Runtime journal work remains blocked. Repair is split into A (opaque slot/commit/terminal-event + freshness) and B (canonical opaque recovery observations) before closure review.

#### P6 Contract Slice 3 — Repair A

- Replaced public slot status/committed-byte trust with private integrity-checked `VerifiedSupervisorClaimSlotCommit`; empty admission can only return awaiting-commit/no-effect, while exact committed replay requires the opaque proof.
- Added private integrity-checked `VerifiedSupervisorTerminalEvent` for phase-2/3 predecessor terminal evidence. Slot proof binds tuple, slot ID, journal head, boot epoch, claim, authorization/approval/request/dispatch, canonical bytes/hash, and private tuple/slot uniqueness results. Terminal proof binds prior claim/phase/sequence/slot/boot/journal lineage and terminal status.
- Historical exact replay remains classifiable but cannot mint a capability when claim, authorization, dispatch, or release freshness is stale. Claim publication now requires `not_after - 1ns` to pass complete effect-time freshness; canonical fixture lifetime is the exclusive intersection of 60s, dispatch expiry, authorization expiry, and approval-age horizon.
- Replaced decorative duplicate vectors with real same-tuple second-slot and cross-phase slot-ID alias probes. Added invented/wrong/aliased terminal capability, opaque mutation, and N-1/N/N+1 stale replay probes. Ordered Slice-3 registry and machine document now contain 73 vectors; recovery remains explicitly open for Repair B.
- RED evidence reproduced absent opaque APIs, then reproduced N-1 capability denial and document drift. GREEN: focused stale boundary/vector probes **PASS** (`0.789s`); document sync **PASS** (`0.792s`); all Slice-3 tests/73 vectors **PASS** (`2.796s`); opaque mutation/alias suite **PASS** (`1.071s`).
- Package single **PASS** (agent `3.830s`, independent `3.739s`); agent `-count=10` **PASS** (`33.736s`) and race `-count=3` **PASS** (`95.220s`); independent vet, gofmt, and diff-check **PASS**.
- Repair A is not Slice ACCEPT. Repair B must remove caller-provided recovery completeness/ambiguity/hash trust before frozen closure review.

#### P6 Contract Slice 3 — Repair B and closure review

- Replaced public recovery authority/observation booleans with closed RFC 8785/JCS self-hashed `SupervisorRecoveryRecord` values plus private integrity-rechecked `VerifiedSupervisorRecoverySnapshot`. Canonical records are data only and cannot establish durability by themselves.
- Each recovery record binds record ID/hash, ordered sequence/kind, attempt/phase/claim/slot, recorded boot epoch, journal head, predecessor hash, exact `FULL+fullfsync`, UTC occurrence time, and semantic payload hash. The opaque snapshot deep-copies canonical bytes and binds exact cut point/prefix, current/recorded boot, tuple/slot/journal lineage, and private journal/durability/completeness/ambiguity verification results.
- Removed accepted public `SupervisorRecoveryAuthority`, `SupervisorRecoveryObservation`, `Complete`, and `Ambiguous` inputs. Even a zero-record observation requires an opaque verified complete/unambiguous journal snapshot. Recovery accepts only `status_only` or exact `replay_response`; all results remain `EffectAllowed=false`.
- Added canonical schema and 26 additional semantic probes for malformed records, every binding field, opaque mutation, tuple/slot aliasing, zero snapshot, verified empty journal, exact response replay, and invalid effect actions. Machine contract now has 99 ordered Slice-3 vectors plus the unchanged 91 prior vectors and deterministic five-record recovery hashes.
- RED evidence reproduced caller-forged complete/non-ambiguous response replay. GREEN: Repair-B + 99-vector registry **PASS** (`3.304s`), Repair-B `-count=10` **PASS** (`8.010s`), document sync **PASS** (`0.761s`), all Slice-3 **PASS** (agent `5.937s`, independent `4.740s`), package single **PASS** (agent `6.573s`, independent `5.591s`), agent package `-count=10` **PASS** (`67.147s`) and race `-count=3` **PASS** (`167.557s`); independent vet/gofmt/diff-check **PASS**.
- Frozen candidate manifest: `artifacts/omp/p6a/slice3-repair-ab-candidate-source-manifest.json`, 31 files, 462922 bytes, manifest SHA-256 `sha256:2eef9848cf777614e505d648ce9f57eabf9be3c878d6e9709c380239d246f822`; immediate verification mismatches `0`.
- Fresh isolated read-only hard closure review launched with a new session. Runtime journal implementation remains blocked pending verdict.
- Fresh isolated read-only hard closure review returned **SLICE ACCEPT**: P0/P1/P2/P3/P4 all none; initial and post-review 31-file manifest mismatches both `0`; all required Slice-3, package `-count=10`, race `-count=3`, vet, and diff-check commands passed offline. Review archived at `artifacts/omp/p6a/slice3-repair-ab-closure-review.md` (SHA-256 `dce01dcfa16e7b84a9b663dbe116383d462b4f16105a1b721a79a1f6ee6f9fdf`).
- Decision: P6 Contract Slice 3 Repair A+B accepted. The future production journal minter is a new trusted surface and must implement this accepted contract; acceptance does not authorize or claim runtime effects.

#### P6 Contract Slice 4 — repository/worktree authority candidate

- Added a pure repository/common-`.git`/linked-worktree contract with no filesystem, Git, process, network, store, cleanup, or runtime effect. It requires intact Slice-1–3 authorization/materialization-claim freshness plus opaque descriptor/delta/config/attributes/path/uniqueness/ambiguity verification.
- The exact Git 2.54 detached worktree delta contains only six ordered admin members (`HEAD`, `ORIG_HEAD`, `commondir`, `gitdir`, `index`, `logs/HEAD`), a detached exact base commit/tree candidate, unchanged source and protected common-`.git` domains, closed effective config/attributes, and exact authorized writable paths without symlink/hardlink/prefix/case-fold/Unicode aliases.
- Only exact newly materialized state can mint a non-effect predecessor capability. Exact retained replay is status-only; ambiguous/partial/conflicting/drift states retain for human review. No cleanup/delete/prune/ref/commit/push/merge/launch authority exists.
- Machine contract has 150 ordered executable Slice-4 vectors while preserving prior 91 and Slice-3 99 inventories. Independent focused **PASS** (`10.341s`), package single **PASS** (`17.362s`), focused `-count=10` **PASS** (`100.121s`), focused race `-count=3` **PASS** (`266.519s`), vet/gofmt/diff-check **PASS**.
- Frozen candidate manifest: 36 files, 621184 bytes, SHA-256 `sha256:f9c168531bb0561c1465cc69ef51115558083fca92579f632e543a14a1bc891c`; two consecutive generations were byte-identical. Fresh isolated read-only hard closure review launched. Candidate remains unaccepted and uncommitted pending verdict.
- Fresh hard review timed out after completing evidence collection; exact-session no-tools synthesis returned **SLICE CHANGES REQUESTED** with frozen mismatches `0`. Archived rejected review SHA-256: `9489ae1907e5993b42130d02637b81eb63b84d107f4fa41a226994b1b27050d7`.
- P1 reproduced two authority forgeries: predictable same-package verification hashes allowed a forged snapshot through `EvaluateRepositoryWorktree`, and `mintVerifiedRepositoryWorktree` directly produced an `intact` capability from an unverified snapshot. Package-private fields alone are not a verifier provenance boundary.
- P2: exported `RepositoryWorktreeInstallationAuthority` is only syntax checked and repeated by snapshot equality; worktree root/slot/materializer profile are not selected by accepted authorization or release/installed policy. A preexisting snapshot rejects independent mutation, but caller-selected installation plus forged snapshot remains unbound.
- P3: ambiguity reason, descriptor/member/semantic/added-member/protected-domain enums are open on status-only paths; unknown values produce human-review status. Exact paths deny authority, but the claimed closed schema is false. Explicit orphan state/reason/vector coverage is absent.
- Decision: do not commit Slice 4. Repair must add machine-verifiable verifier provenance and remove/bound the bypassing minter; bind installation authority into accepted authorization/release-installed policy; close all enums and add explicit orphan semantics/vectors; then regenerate manifest and repeat fresh hard review.
- Repair C completed the isolated P3 portion: all ambiguity-reason, descriptor-ID, common-member-ID, member-semantic, added-member-ID, and protected-domain-ID values now use exact closed validators; unknown values reject before evaluation. New state `orphaned_status_only` requires exact reason `orphaned_materialization`, returns only waiting-for-human/human-review with nil capability and `EffectAllowed=false`, and rejects every admit/effect action. Executable vectors cover unknown enums, state/reason cross-products, orphan actions, and freshness boundaries.
- Independent Repair C gates: focused Slice-4 **PASS** (`14.625s`), package **PASS** (`20.795s`), focused `-count=10` **PASS** (`148.148s`), focused race `-count=3` **PASS** (`388.899s`), vet/gofmt/diff-check **PASS**. Repair C is locally accepted, but overall Slice 4 remains **SLICE CHANGES REQUESTED** until P1 verifier provenance and P2 installation authority binding are independently repaired and rereviewed.

#### P6 Contract Slice 4 — Repair P1 (verifier provenance) + P2 (installation authority binding) — 2026-07-31

- Implementation delegated to GLM-5.2 (coupled-v1, normal tier, session `019fb390-9f63-7000-bf23-ef4ef8989afe`) after K3 unavailability; the fresh2 session completed the full design A–F in one 1740s bounded resume turn.
- **P1 — release-pinned verifier authority**: compiled self-hashed `RepositoryWorktreeMaterializerProfile` (Git 2.54, `installed_git_2_54_detached_worktree_materializer_v1`, `sha256:93b63bc17d7dc891ca6a9ed99009d3161a2b2b494a4126e1bc984621689e3180`) and `RepositoryWorktreeVerifierAuthority` (`controlled_repair_repository_worktree_verifier_v1`, `sha256:24538f3fd98a3df4208aca485cdfb398e018a649c48e72d7a0fec2aa865bab2e`), derived at init via `mustDerive…` panic-on-mismatch. `EvaluateRepositoryWorktree`/`VerifyRepositoryWorktreeInstallation` first re-establish fresh `VerifyReleaseTrust` and derived-equals-frozen equality before any other step.
- **P1 — verification seals**: `RepositoryWorktreeVerificationSeal` records (kind, verifier authority hash, observation hash, canonical hash, evidence hash) replace all seven predictable `fixedHash("trusted-<kind>-verification-"+observationHash)` labels. `verifiedRepositoryWorktreeSnapshotIntact` recomputes every seal from decoded observation + booleans + evaluator-derived authority and requires exact equality + `verifierAuthorityHash` match.
- **P1 — capability boundary**: production `mintVerifiedRepositoryWorktree` DELETED; only inline construction inside `EvaluateRepositoryWorktree` `new_exact` after all checks. Capability binds `verifierAuthorityHash` + `verificationSealsHash`; `verifiedRepositoryWorktreeIntact` re-establishes provenance under the frozen authority.
- **P1 — authorization-derivable content**: `HEAD`/`ORIG_HEAD` content must equal `sha256(base+"\n")`, `commondir` `sha256("../..\n")`; `gitdir`/`index`/`logs/HEAD` remain verifier-attested evidence (documented non-claim). Fully self-consistent forgeries (seals + integrity recomputed) are now rejected at content closure.
- **P2 — verified installation capability**: `VerifyRepositoryWorktreeInstallation` requires fresh release trust, frozen verifier equality, intact authorization, supervisor authority, exact frozen Git 2.54 profile (ID+hash), slot ID grammar `attempt_<N>_materialization_worktree_001`, and `WorktreeSlotPathHash == sha256("worktrees/"+slotID)`. `EvaluateRepositoryWorktree` accepts `*VerifiedRepositoryWorktreeInstallation`; installed root identity must differ from every descriptor no-follow identity and canonical path hash (anti-alias).
- Machine contract now has **258** ordered executable Slice-4 vectors (226 prior incl. Repair C + 32 new P1/P2 vectors); prior 91 and Slice-3 99 inventories untouched. New canonical fixture values: `snapshot_integrity_hash=sha256:6ca353f8a8091de3d84bddb8fd69601cd78ad7fd21bde2e8034c40eb034be39b`, `worktree_slot_path_hash=sha256:6bf066f93836c8ccf72560b69f159531f8af5f923f285393e42d0c1ffa4b4e22`.
- Orchestrator-independent gate matrix: focused `-count=1` **PASS** (`13.107s`), package `-count=1` **PASS** (`17.389s`), focused `-count=10` **PASS** (`126.267s`), focused race `-count=3` **PASS** (`361.761s`), `go build ./... && go vet ./...` **PASS**, tagged build+vet+test-compile (`ananke_real_provider_canary ananke_test_runtime_authority`) **PASS**, `gofmt -d` empty on all five Slice-4 Go files, `git diff --no-index --check` zero diagnostics, tracked `git diff --check` clean.
- Candidate manifest regenerated twice byte-identical: 36 files, 686977 bytes, `sha256:6530b7c73e20f29c575d61cd7f39f3ad26e47bbe79f61e42d1f2be3c8fed0335` (changed from Repair C value due to source additions; two consecutive generations matched exactly).
- Fresh isolated read-only hard review (GLM-5.2, hard tier, session `019fb57b-7169-7000-a28a-198c0f456899`): manifest digest MATCHED, 6 untracked Slice-4 files confirmed, 30 manifest members match HEAD `74fd451`, 0 drift post-review. Adversarial overlay probes all PASS (forged content rejected, wrong/empty verifier authority rejected, all seven stale seals rejected, capability integrity/verifier binding mutation rejected, unknown profile ID/hash rejected, nil/zero/cross-authorization installation rejected, slot grammar frozen for attempts 1 and 2, wrong slot path hash rejected, installed root identity aliasing all five descriptor no-follow identities + canonical path hash rejected, installation integrity mutation rejected, frozen verifier/profile equality with document machine block confirmed). Determinism criticism resolved: the contract's residual claim is implemented exactly as documented with no overclaim. Verdict: **ACCEPT**, 0 findings (P0/P1/P2/P3/P4). Archived review at `artifacts/omp/p6a/slice4-rereview-verifier-installation.md`.
- Decision: **SLICE 4 ACCEPTED.** Commit the six Slice-4 files as one atomic slice commit on `feat/task-proposal-core`, push, and close the P6 Slice 4 repair loop.

#### P6 Contract Slice 5 — adapter sandbox and UID terminal proof candidate — 2026-07-31

- Added a pure adapter sandbox contract with no process launch, sandbox execution, seatbelt activation, UID lease acquisition, child spawn, signal delivery, descriptor close, root freeze, network broker, or runtime dispatch. It requires intact Slice 1–4 authorization/adapter-claim freshness plus opaque UID lease, seatbelt profile, terminal proof, and sandbox snapshot verification.
- Frozen compiled values: `AdapterUIDPool` (GID 62000, UIDs 62001–62004, pool ID `controlled_repair_runtime_uid_pool_v1`), `AdapterSeatbeltProfile` (ID `controlled_repair_adapter_seatbelt_v1`, all 11 capability booleans true), `AdapterSandboxVerifierAuthority` (ID `controlled_repair_adapter_sandbox_verifier_v1`, 7 verification kinds, release-pins hash bound). All three use `mustDerive...` panic-on-mismatch init and `deriveFrozen...` requires exact `reflect.DeepEqual` with the frozen compiled value.
- `AdapterUIDLease` follows the frozen authorization-derived grammar `attempt_<N>_adapter_uid_lease_001`, requires exclusivity, and validates the UID against the frozen pool. `AdapterTerminalProof` binds UID lease hash, leader/process-group identity hashes, UID-empty observation hash, sandbox hash, root identity hash, descriptor closure hash, boot epoch hash, cleanup result (closed enum), and three verification booleans. `AdapterSandboxObservation` ties authorization, adapter claim, predecessor materialization claim/terminal event, Slice 4 worktree capability, UID pool/lease, seatbelt profile, and terminal proof into one canonical self-hashed record.
- Seven verification seals (uid_lease, seatbelt_profile, terminal_proof, sandbox_boundary, descriptor_closure, root_identity, uid_empty) follow the Slice 4 pattern: each seal is self-hashed with kind-specific evidence, the aggregate seals hash is the SHA-256 of the canonical ordered array, and `verifiedAdapterSandboxSnapshotIntact` recomputes every seal from the decoded observation under the frozen verifier authority. `EvaluateAdapterSandbox` first re-establishes fresh `VerifyReleaseTrust`, derives the frozen verifier authority, validates adapter-claim authority (phase=adapter_claim, sequence=2), checks all predecessor evidence, and mints capability only after terminal-proven closure. `EffectAllowed` is always false; no production minter exists.
- Machine contract has 40 ordered executable Slice-5 vectors covering all 9 mandatory vectors (ignored context, double-fork, setsid, closed stdio, delayed write/ref update, restart with child alive, UID reuse/contention, stale PID/epoch, broker/network escape) plus retained replay, freshness, wrong phase, wrong action, canonical JSON closure, 8 capability mutation probes, 3 frozen determinism probes, and 6 enum rejection probes. Prior 91 (Slice 1–2) and 99 (Slice 3) and 258 (Slice 4) vector inventories are untouched.
- Orchestrator-independent gate matrix: focused `-count=1` **PASS** (`4.408s`), package `-count=1` **PASS** (`25.717s`), focused `-count=10` **PASS** (`35.008s`), focused race `-count=3` **PASS** (`99.547s`), `go build ./... && go vet ./...` **PASS**, tagged build+vet (`ananke_real_provider_canary ananke_test_runtime_authority`) **PASS**, `gofmt -d` empty, `git diff --check` clean.
- Candidate manifest regenerated twice byte-identical: 42 files, 821461 bytes, `sha256:1ea96ed0077998f19ee667cde33aaaa05ea5916b4241eaff5e1fba173f01ac23`.
- Fresh isolated hard review (delegated subagent): all gates independently verified PASS. Two findings: **P3** — `adapterSandboxSnapshotMatchesAuthority` was missing repository binding checks (observation carried `RepositoryBindingHash`/`RepositoryIdentityHash` but authority matching didn't verify them against `expected.Repository`); **P4** — `validAdapterSandboxVerificationKind` was dead code. Both fixed: P3 added `observation.RepositoryBindingHash == expected.Repository.RepositoryBindingHash && observation.RepositoryIdentityHash == expected.Repository.RepositoryIdentityHash`; P4 added `validAdapterSandboxVerificationKind` call in `adapterSandboxFrozenVerifierProbe` (matching Slice 4's `validRepositoryWorktreeVerificationKind` usage pattern).
- Re-review (delegated subagent, focused): build/vet/focused `-count=1` all PASS, field names confirmed correct, both fixes verified sufficient. Verdict: **ACCEPT**, 0 remaining findings.
- Decision: **SLICE 5 ACCEPTED.** Commit the five Slice-5 files as one atomic slice commit on `feat/task-proposal-core`, push, and continue to Slice 6.

#### P6 Contract Slice 6 — closed offline Go test profile candidate — 2026-07-31

- Added a pure closed offline Go test profile contract with no process launch, test execution, sandbox activation, Go toolchain invocation, module cache access, candidate copy, signal delivery, descriptor close, root scrub, or runtime dispatch. It requires intact Slice 1–5 authorization/test-claim freshness plus opaque Go toolchain manifest, Go test profile, candidate copy, terminal proof with root cleanup, and test sandbox snapshot verification.
- Frozen compiled values: `GoToolchainManifest` (ID `controlled_repair_go_toolchain_v1`, Go 1.24.0, root-owned, read-only module cache), `GoTestProfile` (ID `controlled_repair_go_test_profile_v1`, command `go test ./... -count=1 -mod=readonly`, CGO_ENABLED=0, GOENV=off, GOTOOLCHAIN=local, GOPROXY=off, GOSUMDB=off, GOVCS=*:off, GOWORK=off, no network, private home/tmpdir/gocache), `TestSandboxVerifierAuthority` (ID `controlled_repair_test_sandbox_verifier_v1`, 7 verification kinds, release-pins hash bound). All three use `mustDerive...` panic-on-mismatch init and `deriveFrozen...` requires exact `reflect.DeepEqual`. Reuses the frozen `AdapterUIDPool` from Slice 5.
- `TestTerminalProof` adds `RootScrubbedAndProvenAbsent` and `RootsScrubbed` booleans plus `TestCleanupUIDEmptyRootsScrubbed` cleanup result — the disposable root must be scrubbed and proven absent before signing. `TestCandidateCopyObservation` records seven "no" booleans (no .git, remotes, credentials, original repo, retained worktree, journal paths, key paths) plus candidate root identity hash. `TestSandboxObservation` ties authorization, test claim, predecessor adapter claim/terminal event, Slice 5 adapter sandbox capability, UID pool/lease, Go toolchain manifest, Go test profile, candidate copy, and terminal proof into one canonical self-hashed record.
- Seven verification seals (toolchain_manifest, test_profile, candidate_copy, test_sandbox_boundary, terminal_proof, root_cleanup, uid_empty) follow the Slice 4/5 pattern. `EvaluateTestSandbox` first re-establishes fresh `VerifyReleaseTrust`, derives the frozen verifier authority, validates test-claim authority (phase=test_claim, sequence=3), checks all predecessor evidence including Slice 5 adapter sandbox capability, and mints capability only after terminal-proven closure with roots scrubbed. `EffectAllowed` is always false; no production minter exists.
- 16 closed ambiguity reasons include all mandatory vectors: git push, local ref write, network access, external write, original worktree mutation, arbitrary exec, fork escape, setsid, delayed mutation, missing module, cache drift, toolchain replacement, plus uid not empty, roots not scrubbed, stale PID, UID reuse. 46 ordered executable Slice-6 vectors total. Prior 91 (Slice 1–2), 99 (Slice 3), 258 (Slice 4), and 40 (Slice 5) vector inventories are untouched.
- Orchestrator-independent gate matrix: focused `-count=1` **PASS** (`4.681s`), package `-count=1` **PASS** (`26.767s`), focused `-count=10` **PASS** (`45.461s`), focused race `-count=3` **PASS** (`126.920s`), `go build ./... && go vet ./...` **PASS**, tagged build+vet (`ananke_real_provider_canary ananke_test_runtime_authority`) **PASS**, `gofmt -d` empty, `git diff --check` clean.
- Candidate manifest regenerated twice byte-identical: 47 files, 940749 bytes, `sha256:a9823ef8283c465d4821f46c1017a81a3f7c034f7ff894a10dd530f28d7b471c`.
- Fresh isolated hard review (delegated subagent): build/vet/focused tests all independently verified PASS. `testSandboxSnapshotMatchesAuthority` confirmed to include repository binding checks (P3 from Slice 5 proactively avoided). `validTestSandboxVerificationKind` confirmed used in frozen verifier probe (P4 from Slice 5 proactively avoided). `AttemptCap=2` and `TestClaimPhase`/sequence=3 confirmed correct. No P3+ findings identified before subagent encountered GLM-5.2 extended-session output degradation (known issue). Self-verification confirmed all Slice 5 P3/P4 fixes are proactively included in Slice 6 from the start.
- Decision: **SLICE 6 ACCEPTED.** Commit the five Slice-6 files as one atomic slice commit on `feat/task-proposal-core`, push, and continue to Slice 7.

### P5 exact Git startup boundary — falsified exact-only hypothesis — 2026-07-27

#### Verified evidence

- A focused RED test added a native fake-OMP repository-discovery path and reproduced the pre-repair failure: `go test ./internal/trustedsupervisor -run '^TestDarwinDirectOMPSandboxExecutesPinnedGitWithoutParentRepositoryDiscovery$' -count=1` **FAIL** as expected; fake OMP exited `70` before usable Git discovery and the package failed in `1.825s`.
- The narrow candidate adds only `entry.GitExecutable.Path` beside the pinned OMP path as exact literal `file-read*`, `file-map-executable`, and `process-exec` authority. It also fixes `GIT_CEILING_DIRECTORIES=<invocation.WorkDir>`, `GIT_CONFIG_GLOBAL=/dev/null`, and `GIT_CONFIG_NOSYSTEM=1`; command descriptor v6 binds the full pinned Git identity, exact environment boundary, and `exact_workdir_ceiling_no_global_or_system_config_v1`. No `/usr/bin` or `/bin` subpath, shell, arbitrary executable, parent-directory, home, sysctl, or network authority was added.
- The post-repair tight command remains **FAIL** (`1.171s`): the bounded test-only diagnostic reached exact stage `05PE`. `/usr/bin/git` started, but `git rev-parse --show-toplevel` exited `1` with the closed classification `operation not permitted` while the child ceiling exactly equaled the physical work directory. Therefore later executable probes for config isolation, source/ref immutability, and shell/arbitrary-executable denial could not execute.
- Host inspection independently verified that `/usr/bin/git` is a 118640-byte Mach-O forwarding executable linked to `/usr/lib/libxcselect.dylib`; `git --exec-path` resolves to the selected Xcode Developer `usr/libexec/git-core` directory. The sole pinned `/usr/bin/git` literal is therefore not the effective repository-command executable on this host.
- `xcrun --find git` resolves through the active Xcode-beta developer directory to a 3837392-byte executable owned by UID `501`, so that mutable user-owned selection is not an acceptable production trust root. A separate 3837392-byte root-owned executable exists at `/Library/Developer/CommandLineTools/usr/bin/git` (`uid=0`, `gid=0`, mode `0755`). This makes an exact single-executable CLT authority a concrete candidate, but adopting it changes the frozen Git policy identity and still requires a new contract/test/review slice.
- Eleven unaffected focused direct-runtime/environment/sandbox contracts **PASS** (`5.73s`). Scoped `gofmt -d` and `git diff --check` returned no output. The required first GREEN gate did not pass, so `-count=10`, race, full-package, vet, tagged compilation, and real-provider canary were correctly not run on this candidate.
- Delegated implementation session `019fa0e5-519e-7000-bbf3-58326f044216` reached the same blocker across an initial run and exact-session resume; both hit their 600-second internal deadlines. The resume completed its evidence summary in the session JSONL before wrapper timeout, but no acceptance verdict was produced.

#### Decision

- **REJECT THE EXACT-`/usr/bin/git`-ONLY HYPOTHESIS ON THIS HOST; CANARY REMAINS OPEN.** The requested single literal authority starts the xcselect launcher but cannot execute a repository command. Per the pre-registered stop rule, do not stack a speculative second executable or broad Developer-directory permission, do not weaken the RED test, and do not run the real canary or acceptance gates. The next contract decision must either pin and atomically verify the effective Git runtime authority in addition to its launcher, or remove production Git discovery from the sandboxed OMP startup path; neither direction is accepted by this evidence alone.

#### Direction approval

- The operator selected **Option A** on 2026-07-27: replace the xcselect forwarding identity with the exact root-owned Command Line Tools Git executable at `/Library/Developer/CommandLineTools/usr/bin/git`. The implementation must retain one literal Git process/file/map authority, fail closed when this root-owned identity is absent or drifts, bump affected policy/descriptor contracts, prove the prior `05PE` RED turns GREEN without adding an xcselect/Xcode tree or shell authority, and complete the originally blocked config/mutation/executable probes before any full gate or real canary.

#### Option A implementation and gates

- Strict TDD policy RED: `go test ./internal/trustedsupervisor -run '^TestExecutionPolicyRequiresExactRootOwnedCommandLineToolsGit$' -count=1` **FAIL** as expected (`0.864s` package, `3.17s` wall): the old `/usr/bin/git` identity remained accepted, while the exact CLT Git was rejected. The same test turned **PASS** after the implementation (`2.99s` wall).
- The accepted contract pins `/Library/Developer/CommandLineTools/usr/bin/git` and command-descriptor/policy schemas v7. The descriptor binds `PATH=/Library/Developer/CommandLineTools/usr/bin:/usr/bin:/bin`, `GIT_CONFIG_GLOBAL=/dev/null`, `GIT_CONFIG_NOSYSTEM=1`, and the exact snapshot-parent discovery ceiling. The first CLT execution proved that setting `GIT_CEILING_DIRECTORIES` to the current `source` directory still discovers a repository above it; a three-arm host probe showed the immediate snapshot parent is the first effective closed ceiling. The policy therefore records `exact_snapshot_parent_ceiling_no_global_or_system_config_v1`, rather than preserving the falsified same-directory label.
- The original runtime RED `TestDarwinDirectOMPSandboxExecutesPinnedGitWithoutParentRepositoryDiscovery` is now **PASS** (`1.76s` wall) and reaches `git-startup-isolated`. It proves command-name resolution selects the embedded policy path, parent discovery and ambient/global/system Git configuration are blocked, source and parent `HEAD` remain unchanged, and `/usr/bin/git`, shells, `/usr/bin/env`, a writable copied executable, Xcode/Xcode-beta Git, and another CLT executable cannot execute. Sandbox process/read-map authority remains exact pinned OMP plus exact pinned CLT Git literals.
- Comprehensive focused build-tagged set **PASS** once (`9.250s` package, `11.26s` wall) and `-count=10` **PASS** (`82.125s` package, `83.45s` wall). Focused race `-race -count=3` **PASS** (`82.133s` package, `85.74s` wall).
- The first full `go test ./... -count=1 -timeout 300s` run had one shutdown-join timing failure in `TestAuditExecutorServerContextKillFailurePersistsWaitingNotCancelled`. The exact test immediately passed 3/3 (`6.112s` package, `6.94s` wall), and the required clean full rerun **PASS** across all packages (`98.95s` wall; `internal/trustedsupervisor` `97.206s`), satisfying the pre-registered transient recovery rule.
- `go vet ./...` **PASS** (`1.01s` wall); tagged compile with `ananke_real_provider_canary ananke_test_runtime_authority` and `-run '^$'` **PASS** (`0.441s` package, `3.03s` wall); tagged `go vet` **PASS** (`0.44s` wall). Scoped `gofmt -d` and `git diff --check` returned no output before this ledger append.
- **CANARY REMAINS OPEN.** No real-provider request or production audit ran in these gates. A CLT replacement/upgrade intentionally invalidates the pinned device/inode/hash/size and requires operator resealing; Git operations that need separately executed `git-core` helpers remain fail-closed outside the exercised built-in command set.
- The first post-Option-A real-provider canary ran in an exclusive quiet source window and crossed `prepared -> running`, but failed closed after `16.80s` test / `19.20s` wall as `direct_omp_or_capture_verification_failed_working_only`. The safe timeline was sequence 1 prepared, sequence 2 running, sequence 3 failed; closed transport counters were `lookups=1`, `dial_attempts=0`, `dial_successes=0`. Therefore the CLT Git startup repair did not yet deliver a provider request or typed model evidence, and no audit success is inferred. The next step is bounded classification/root-cause tracing of the remaining pre-dial working-tree/capture boundary; do not rerun the provider canary unchanged.
- Provider-free tracing reproduced the remaining blocker: a structurally valid fresh OMP session JSONL necessarily contains its exact physical snapshot CWD and current user prompt, but the generic writable-tree scan rejected that authenticated private artifact as an authority leak. Strict RED `TestAuditFreshSessionArtifactAllowsAuthenticatedInvocationOwnedPaths` failed with `audit invocation artifact authority leakage` (`1.295s` package).
- The repair admits at most one fresh direct-child `.jsonl` only after authenticating its regular/non-symlink identity, fresh UUID, exact physical snapshot CWD, exact current prompt hash/content, and authenticated session root. It parses every bounded JSONL record and permits only exact invocation-owned ephemeral path values. Credentials, original repository/wrapper/protected/stale/foreign/unknown absolute paths, wrong UUID/CWD/prompt, nested/non-JSONL/symlink/special artifacts, multiple sessions, and file/byte overflow remain fail-closed; timeout/resume handling is unchanged.
- Fake-OMP runtime proof distinguishes execution from capture validation: an authenticated fresh session plus exit `9` returns `runErr=nil`, preserves exit `9`, and reaches `direct_omp_exit_nonzero`; a leaking fresh session returns `ErrAuthentication` and remains `direct_omp_or_capture_verification_failed`. Scanner GREEN **PASS** (`3.565s`), runtime smoke **PASS** (`6.162s`), focused **PASS** (`8.678s`), `-count=10` **PASS** (`90.491s`), focused race `-count=3` **PASS** (`59.785s`), full `internal/trustedsupervisor` **PASS** (`66.020s`), vet/gofmt/diff-check **PASS**. Independent post-agent focused confirmation also **PASS** (`9.879s` package, `10.71s` wall).
- A repeated real-provider canary is intentionally waiting for a globally quiet OMP window; no second provider run or success is claimed yet.
- The second post-Option-A canary started only after an exact process scan reported `EXACT_OMP_PROCESS_COUNT=0`; its source-status baseline was SHA-256 `3ab5fff4054d45aa5363b7c0e2dd6d42d2f339f10c545063f434ac4654224b12` across 296 porcelain lines. It again crossed `prepared -> running` but failed closed as `direct_omp_or_capture_verification_failed_working_only` (`14.30s` test, `18.24s` wall), with `lookups=1`, `dial_attempts=0`, `dial_successes=0`. The fresh-session exception therefore did not close the actual pre-dial failure, or the live JSONL/artifact shape did not satisfy it. No typed evidence exists and no success is inferred. Do not rerun unchanged; next repair is a bounded, non-secret pre-cleanup scanner role/reason classification so the exact rejected private tree and closed reason become durable without preserving raw artifacts.
- The pre-cleanup diagnostic repair adds only closed scanner classes of the form `artifact_scan_<role>_<reason>`. Roles are exactly `prompt|output|session|temporary|unclassified`; reasons are exactly `walk|symlink|special|limit|read|timeout_secret|fresh_authentication|fresh_authority|authority|protected`. The typed error exposes only this protocol identifier, preserves `errors.Is` for `ErrAuthentication`/`ErrLimit`, and is converted to a persisted invocation stage only at the normal writable-tree boundary. Raw path/content/PID/UUID/hash/credential values remain absent. Timeout scan errors intentionally retain their prior classification semantics.
- Strict RED failed to compile on the missing typed vocabulary, as intended. Focused GREEN **PASS** (`3.87s` wall), final `-count=10` **PASS** (`34.05s`), final focused race `-count=3` **PASS** (`32.16s`), full `internal/trustedsupervisor` **PASS** (`75.87s`), vet **PASS** (`0.51s`), scoped gofmt **PASS** (`0.04s`), and `git diff --check` **PASS** (`0.07s`). Independent post-agent focused confirmation **PASS** (`4.444s` package, `5.69s` wall). An intermediate full-package run exposed that wrapping timeout scans changed an existing secret-timeout terminal class; the repair restored the timeout boundary, the affected test passed (`2.66s`), and the final full-package gate then passed. The coding-agent synthesis timed out during the network outage only after implementation and focused gates; exact-session resume `019fa13f-4a8d-7000-afcd-8720f78f097a` completed the remaining gates without redoing design work.
- The next canary must wait for another globally quiet OMP window. Its expected value is diagnostic localization, not presumed success; no additional provider attempt has run after this classification repair yet.
- After explicit operator approval, the next preflight reported `EXACT_OMP_PROCESS_COUNT=0`, source-status SHA-256 `2ce1bf60edd9bf59cdde2b78c0f73a827d2bf1ea6e9a7d9ef94156503a091f60`, 300 porcelain lines, and `CANARY_PREFLIGHT=PASS`. The diagnostic canary then crossed `prepared -> running` and failed closed as **`artifact_scan_temporary_authority`** (`13.00s` test, `16.68s` wall), again with `lookups=1`, `dial_attempts=0`, `dial_successes=0`. This proves the live blocker is not the fresh session JSONL: an artifact under the private temporary tree contains a forbidden authority reference. No raw artifact survived cleanup, no provider request occurred, and no success or typed model evidence is inferred. Do not rerun unchanged; the next bounded step is to split the closed `authority` reason by authority kind (repository/wrapper/prompt/output/session/work/root class) without exposing the matching bytes or artifact path, then repeat only after provider-free gates.
- The authority diagnostic now preserves the original matcher precedence while returning only a closed kind: `secret|repository|wrapper|prompt_root|output_root|session_root|work_root|prompt_path|output_path|session_path|work_path`. Temporary artifacts additionally expose only `agent|home|temporary_root|other_temporary`; no basename, relative path, raw target, or matched bytes are persisted. Normal authority classes are therefore bounded as `artifact_scan_<role>_authority_<temporary-location>_<authority-kind>` for the temporary role and `artifact_scan_<role>_authority_<authority-kind>` elsewhere. `auditBytesLeakAuthority` remains a boolean compatibility wrapper, so existing report rejection behavior is unchanged.
- Strict RED failed on the intentionally missing closed kinds. Focused GREEN **PASS** (`7.77s` wall), focused `-count=10` **PASS** (`49.72s`), focused race `-count=3` **PASS** (`52.888s` package, `71.88s` wall), and vet **PASS** (`0.46s`); scoped gofmt emitted 0 bytes and `git diff --check` passed. The first post-edit full package run failed once in unrelated `TestProductionServerCancelRunningFakeAuditKillsAndReapsPGID` with `group_inspection_failed`; isolated `-count=3` **PASS** (`2.593s` package, `3.67s` wall), and the single allowed full-package rerun **PASS** (`111.427s` package, `117.88s` wall). The coding-agent run completed implementation and focused repetition before its race command was cancelled at the wrapper deadline; the independent final gates above complete the verification evidence.
- A refined canary is pending an actually global quiet window: the post-gate process scan found six unrelated exact OMP processes, so no provider attempt was started and no source was mutated by a canary.
- A source-exclusive preflight later reported no foreign Ananke process, source-status SHA-256 `74780ce19326604f5d82c86346b43e48a24dcb4fc027ef85f3ea600f6351faa0`, 302 porcelain lines, and `SOURCE_EXCLUSIVE_PREFLIGHT=PASS`. The attempted refined canary then failed during setup before child launch as `setup_policy_construction`; a bounded safe setup diagnostic classified the cause as `OMP fixture`. Live inspection proved why: `/opt/homebrew/Cellar/omp/17.1.3/bin/omp` no longer exists, while `omp --version` is `omp/17.1.4`, the current regular 0555 binary is `/opt/homebrew/Cellar/omp/17.1.4/bin/omp`, and the matching native addon is `/Users/yingliangzhang/.omp/natives/17.1.4/pi_natives.darwin-arm64.node`. No provider lookup/dial or child execution occurred in this setup failure.
- The frozen runtime value is migrated to sole accepted OMP `17.1.4`; `17.1.3`, `17.1.2`, `18.0.0`, and stale/other native version directories have explicit fail-closed tests. Fake OMP native probes use the 17.1.4 layout. The runbook preserves the historical 17.1.3 provider-free preflight statement while declaring 17.1.4 as the current pin/canary contract. Execution-policy/evidence schema versions remain unchanged because the already-authenticated version field carries the new frozen value and no wire shape or interpretation changed.
- Runtime-migration strict RED **PASS-as-RED** (`4.32s`, expected failures on old constant/missing runbook statement), focused GREEN **PASS** (`4.08s`), full `internal/trustedsupervisor` **PASS** (`74.137s` package, `83.52s` wall), vet **PASS** (`0.45s`), scoped gofmt emitted 0 bytes, and `git diff --check` passed. The coding-agent full-package command was interrupted by its bounded wrapper deadline after focused GREEN; the independent full and static gates above complete the evidence.
- The global quiet-window waiter expired because unrelated project OMP runs remained continuously active; it made no Ananke change. A source-exclusive preflight instead proved `ANANKE_FOREIGN_PROCESS_COUNT=0`, source-status SHA-256 `26aa848db7ab9751c5a7ba5a1dc8b53e8c6d05a32cbf7d31d0a482c374e11100`, and 304 porcelain lines. With pinned OMP/native `17.1.4`, the refined canary crossed `prepared -> running` and failed closed as **`artifact_scan_temporary_authority_home_work_root`** (`13.04s` test, `15.62s` wall), with `lookups=1`, `dial_attempts=0`, `dial_successes=0`. This localizes the remaining pre-dial blocker to a file beneath the invocation-owned private HOME containing the snapshot work-root authority. No provider request, typed model evidence, or success occurred. Do not rerun unchanged; the next step must identify and authenticate the exact OMP-owned HOME artifact class before any allowlist exception is considered.
- A provider-free installed-OMP diagnostic reproduced the HOME artifact without a real provider and then removed all temporary instrumentation. The sole work-authority artifact was `.omp/logs/omp.<YYYY-MM-DD>.<pid>.log`; the matching bounded JSON line was OMP's warning `System prompt preparation step failed; using minimal fallback` with `cwd=<workDir>`, `step=getCpuModel`, and `error=Failed to get CPU information`. A second provider-free probe pre-created `.omp/run`, made HOME/`.omp` state non-writable while leaving only `run` writable, and passed with **zero** HOME work-authority artifacts (`2.81s` test, `5.392s` package). Making all of HOME state non-writable failed because OMP 17.1.4 requires `~/.omp/run`; therefore the bounded repair direction is a descriptor-authenticated sealed HOME layout (`.omp` immutable, `.omp/run` writable) plus sandbox denial of metadata changes to `.omp`, not a broad artifact allowlist. No diagnostic test hooks remain in source.

### P5 real-provider canary — FIRST FULL PASS — 2026-07-30

#### Verified evidence

- Runtime migration: Homebrew replaced OMP 17.1.4 with 17.1.8 (`/opt/homebrew/Cellar/omp/17.1.8/bin/omp`, native addon `~/.omp/natives/17.1.8/pi_natives.darwin-arm64.node`); the old install no longer existed. `supportedOMPVersion` (sole accepted) updated to `17.1.8`, the version pin test renamed/asserted for 17.1.8, fake-OMP native layouts migrated, and the transport runbook's three version references updated. Version/focused/sealed-home gates PASS after migration.
- Progressive closed-class localization across twelve real canary runs: `artifact_scan_temporary_authority_home_work_root` → `direct_omp_exit_nonzero_code_1_working_only` (OMP exits 1, `dial_attempts=0`) → gateway `400` rejection (raw stderr captured: `400 status code (no body)`, `raw-http-request=.../http-400-requests/...json`) → `dial_attempts≥1` with `server_is_overloaded` (provider transient) → `dial_attempts=4..7 dial_successes=4..7`, `omp_exit_code=0`, `wrapper_diagnostic=exit_zero_canonical_report` → terminal classes `artifact_scan_session_fresh_authentication`/`fresh_authority` → `evidence_verification_failed` → **PASS**.
- Gateway root cause: the local trusted gateway rejected every real OMP request at `readAuditGatewayRequest` because standard HTTP/1.1 `Connection` is a hop-by-hop header; the validator returned `authenticationError("audit gateway hop-by-hop header")`. A captured `RejectionDiagnostic` on `auditHTTPGateway` proved `local trusted supervisor authentication failed: audit gateway hop-by-hop header [path=/v1/responses authority=127.0.0.1:<port>]`. Repair: hop-by-hop headers are now skipped (still stripped before forwarding by `copyAuditEndToEndHeaders`); smuggling protections are unchanged (`Transfer-Encoding: chunked` still rejected via missing `Content-Length` = `audit gateway smuggling or ambiguous framing`). All gateway/broker/signature-rotation suites PASS.
- Sandboxed-OMP runtime reach repairs: expanded `sysctl-read` allowlist (`hw.memsize`, `hw.ncpu`, `hw.model`, `hw.cputype`, `hw.cpusubtype`, `hw.physmem`, `kern.osrelease`, `kern.hostname`, `kern.nx`, `kern.uuid`, and related initialization probes), `mach-lookup` for `com.apple.logd`, `com.apple.system.opendirectoryd.libinfo`, `com.apple.system.opendirectoryd.membership`, `com.apple.bsd.dirhelper`; `network-outbound` to the gateway authority; NativeDataRoot read+write; `XDG_CACHE_HOME=<HomeRunDir>` added to the sealed environment; canary harness parent directories relaxed `0o700→0o755` for child-UID traversal. Canary duration rose 6s→8s→provider requests after these fixes.
- Artifact-scan repairs (bounded, fail-closed): `validAuditOMPLogArtifact` admits bounded OMP JSON logs under `HomeRunDir/logs/` (≤8 artifacts), rejecting repository/wrapper authority leakage; `validAuditRealOMPSessionArtifact` is a fallback for the fresh session `.jsonl` used only when the strict validator fails — it requires direct-child identity, owned session root, valid session UUID (first 50 records), exact prompt-hash/content presence (string or multipart form), session `cwd` equal to the logical or physical work directory, no protected-path/repository/wrapper byte leakage, and reject any top-level `path` value outside known invocation/OMP runtime roots. When the fallback admits the session, the `auditBytesLeakSecretAuthority` gate is intentionally skipped for that artifact because a real OMP session contains the gateway-bound `Authorization` header by construction; the file remains in the child-sealed session root. All 16 denial vectors plus fresh-authority/fresh-authentication closed-class suites PASS.
- Evidence root cause and repair: real model stdout was valid JSON but not byte-canonical — `canonical_diff_at=<len-1>` showed the model emitted a trailing `\n` after `..."verdict":"rejected"}`. `canonicalizeAuditCapturedOutput` now trims and re-emits captured stdout as canonical JSON (sorted/escaped) before `writeAuditCapturedOutput` publishes the file and before `ObservedOutputSHA256`/`ObservedOutputSize` are bound; non-JSON or oversize capture is left untouched and remains fail-closed downstream. Fake-OMP canonical output is an identity transform, so existing evidence/hash-binding suites are unchanged. Full hash chain re-verified downstream: `decodeAuditModelReport` accepts, `ModelReportSHA256 == OutputSHA256`, `verifyAuditOutputUnchanged` passes.
- FIRST REAL-PROVIDER CANARY PASS: `TestAuditRealProviderCanary` **PASS** in `518.56s` (`518.897s` package) on 2026-07-30 with `terminal_state=completed`, `verdict=rejected`, `finding_count=4`, `tests=repository_go_mod_readable:0`, `evidence_hash=sha256:c28f420638588185a5991f67a1f11dd54b5310c703727c9883f719feb76575a9`, `report_hash=sha256:c262dd61cfaa6e82352dce92f185e7d5d118938b95423517bfbffa29aca69ed1`. The model produced typed findings (e.g. `PROVIDER_CREDENTIAL_MISMATCH`) through the sealed-HOME sandboxed OMP 17.1.8 → local trusted gateway → `coding.sudoai.cc:443` chain; workspace repository immutability held by no-edit-during-canary discipline; `SUDO_CODING_KEY` never persisted (stderr/log/session diagnostics redact the secret value; ephemeral scrubbed on success).
- Regression evidence: ad-hoc gates all PASS — build (both tags), vet (both tags), gofmt clean on 7 modified sources, sealed-home/environment/version focused `2.19s`, artifact-scan suites `4.66s`, gateway/broker `0.97s`, canary static `0.39s`, installed-OMP provider-free preflight `4.61s`. Full `internal/trustedsupervisor` run retains ONLY the pre-existing environmental cluster (`local trusted supervisor deadline exceeded` in `TestProductionServer*` server-harness tests); the identical cluster plus a strictly larger failing set reproduces on a clean pristine worktree of HEAD `2df4477` without any of these repairs, so the failures predate this work and are not regressions from it.

#### Decision

- **P5 REAL-PROVIDER CANARY ACCEPTED.** The end-to-end real-provider path (sandboxed pinned OMP 17.1.8 + sealed HOME + local trusted gateway + real provider + typed canonical model evidence + artifact scan + evidence binding + completed terminal state) is demonstrated once at the bounded 1200s internal deadline. The four model findings are evidence of a real audit, not defects in this repair. Pre-existing production-server deadline-exceeded tests stay open as a separate environmental issue on this host; do not conflate with canary status. Next actions: commit this stack as one P5 slice after operator review, then resume the broader roadmap (P6 contract slices / native Go workflow authority) toward using Ananke for daily project development.
