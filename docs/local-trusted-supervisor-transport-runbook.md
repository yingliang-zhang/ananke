...
The installed OMP v17.1.3 provider-free preflight statement above is historical. The current pinned runtime and canary contract accepts only OMP v17.1.8; v17.1.4, v17.1.3 and every other version fail closed before child launch.
...
The execution-policy document has schema version `ananke.local-trusted-supervisor-execution-policy.v7` and a closed `executions` array. Every entry binds its schema version, launch-spec hash, task and repository identities, repository directory identity, the exact root-owned Command Line Tools Git executable `/Library/Developer/CommandLineTools/usr/bin/git`, exact commit/commit-object/tree/archive hashes, fixed prompt template ID/hash, route mapping, verified wrapper compatibility-oracle identity, exact direct OMP executable and native-addon identities, immutable atomic runtime authority, provider/model/normal tier, internal deadline, wrapper grace, attempt cap, and policy hash. It also binds every supervisor-owned allowed test by ID, command hash, executable and executable-root identities, closed arguments, and timeout; the runtime-read and executable roots; the one exact credential environment name; and five isolated roots. Unknown fields, stale schema versions, noncanonical bytes, duplicate launch hashes, mutable or user-owned Git, `/usr/bin/git`, Xcode-selected Git, path drift, symlinks, owner/mode/device/inode/hash drift, broad roots, overlapping roots, and unsupported routes fail closed.

The sole supported direct OMP runtime version is v17.1.8. Its exact executable, native-addon path under the v17.1.8 native layout, file identities, and atomic runtime authority remain policy-bound; v17.1.4, v17.1.3 and every other version are rejected.

The direct command descriptor has schema version `ananke.local-trusted-supervisor-command-descriptor.v7`. It binds the full pinned CLT Git identity, `GIT_CEILING_DIRECTORIES=<exact snapshot parent>`, `GIT_CONFIG_GLOBAL=/dev/null`, `GIT_CONFIG_NOSYSTEM=1`, and the exact child lookup path `/Library/Developer/CommandLineTools/usr/bin:/usr/bin:/bin`. The snapshot parent ceiling is the first ancestor above the `source` working directory, so Git may inspect the snapshot itself but cannot discover the repository rooted at the configured work root above that parent. Ambient `PATH`, Git command configuration, global configuration, system configuration, `DEVELOPER_DIR`, `GIT_EXEC_PATH`, and Xcode selection cannot change that descriptor or child environment.


The server checks owner-only files, pinned directory/file identity, socket mode and ownership, Darwin peer credentials, canonical bounded frames, authorization/predecessor bindings, execution-policy bindings, and durable replay/audit state. It rechecks the policy, wrapper compatibility oracle, direct OMP executable, native addon, Git executable, repository, allowed-test executables, and all pinned roots at effect boundaries. `--timeout` and `--max-frame-bytes` remain optional and default to `2s` and `65536`.
...
9. constructs canonical read-only evidence schema `ananke.local-trusted-supervisor-readonly-audit-evidence.v6`, whose exact ordered `owned_roots` array records `role`, `path`, `parent_path`, child and parent device/inode, owner UID/GID, mode, and `cleanup_root` for every attempt prompt/output/temporary root, nested agent/home/immutable `.omp`/writable `.omp/run` root, and shared session/work/source-snapshot authority; the array contains identities only, never private file contents. Version v6 supersedes v5 because the exact root inventory and parent-mode semantics changed;
...
The v17.1.3 installed-preflight statement above is historical. The current pinned runtime and canary contract accepts only OMP v17.1.8; v17.1.4, v17.1.3 and every other version fail closed before child launch.
